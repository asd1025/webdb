...
from dept_emp de, employees e,titles t where de.emp_no=e.emp_no and t.emp_no=de.emp_no
 and t.to_date>=sysdate() and t.from_date<=sysdate() order by ename ;
 
-- 문제3. 전체 사원의 사번, 이름, 현재 부서를 이름 순서로 출력하세요..
select e.emp_no,concat(e.first_name,' ',e.last_name) as ename ,d.dept_name 
from dept_emp de, employees e,departments d where de.emp_no=e.emp_no and d.dept_no=de.dept_no
 and de.to_date>=sysdate() and de.from_date<=sysdate() order by ename ;
...
where de.emp_no=e.emp_no and d.dept_no=de.dept_no and t.emp_no=de.emp_no
 and de.to_date>=sysdate() and de.from_date<=sysdate() 
 and t.to_date>=sysdate() and t.from_date<=sysdate() order by ename ;


-- 문제5. ‘Technique Leader’의 직책으로 과거에 근무한 적이 있는 모든 사원의
-- 사번과 이름을 출력하세요. (현재 ‘Technique Leader’의 직책(으로 근무하는 사원은 고려하지 않습니다.)
--  이름은 first_name과 last_name을 합쳐 출력합니다.
select e.emp_no,concat(e.first_name,' ',e.last_name) as ename from titles t, employees e where e.emp_no=t.emp_no and
